...
# Model configuration
DEFAULT_MODEL = "claude-3-5-sonnet-20241022"  # Upgraded from Haiku!
...
TOOLS = [
    ...
    {
        "name": "search_drive",
        "description": "Search for files in Google Drive by name or content. Returns file names, types, links, and modification dates. Use this when user asks about documents, files, PDFs, spreadsheets, etc.",
        "input_schema": {
            "type": "object",
            "properties": {
...
def get_conversation_history(chat_id: int) -> list:
    """Get conversation history for a chat, or initialize if not exists."""
    if chat_id not in conversation_history:
        conversation_history[chat_id] = []
    return conversation_history[chat_id]

def add_to_history(chat_id: int, role: str, content: str):
    """Add a message to conversation history, maintaining max length."""
...
async def process_user_message(update: Update, context: ContextTypes.DEFAULT_TYPE, user_message: str, is_voice: bool = False):
    ...
    try:
        ...
        system_prompt = f"""Eres Claudette, una asistente personal inteligente para Pablo en Costa Rica. Respondes en español de manera natural y conversacional.

FECHA ACTUAL: Hoy es {today_str} (febrero 2026).
- "hoy" / "today" = {today_str}
- "mañana" / "tomorrow" = {tomorrow_str}

SIEMPRE usa el año 2026 para fechas.

HERRAMIENTAS DISPONIBLES:
- Calendario: ver y crear eventos
- Gmail: buscar, leer y enviar emails
- Tasks: listar, crear, completar tareas
- Drive: buscar archivos, ver carpetas, archivos recientes
- Memoria: guardar y recuperar información del usuario

INSTRUCCIONES IMPORTANTES:
1. Cuando el usuario pregunte por archivos, documentos, PDFs, etc., USA search_drive inmediatamente
2. Cuando encuentres archivos, SIEMPRE muestra:
   - Nombre del archivo
   - Tipo (PDF, Doc, etc.)
   - Link clickeable
3. Mantén el contexto de la conversación - si el usuario pregunta "cuál PDF?", refiérete a los resultados anteriores
4. Sé proactivo y útil - si encuentras varios archivos, pregunta cuál quiere ver
5. Para información personal del usuario, usa read_local_file con "user_profile.md"

FORMATO DE RESPUESTAS:
- Sé conciso pero completo
- Usa emojis moderadamente para claridad
- Cuando listes archivos, hazlo de forma clara y ordenada
- Incluye siempre los links de Drive para que el usuario pueda abrir los archivos
"""
        
        # Build messages array with history
        messages = get_conversation_history(chat_id).copy()
        
        logger.info(f"🚀 CALLING CLAUDE API ({DEFAULT_MODEL}) with {len(messages)} messages in history...")
                ...
                # Google Drive tools
                    ...
                    if drive_result["success"]:
                        if drive_result.get("files"):
                            files_text = f"🔍 Encontré {drive_result['count']} archivos para '{query}':\n\n"
                            ...
                            result = f"No encontré archivos con '{query}'. Intenta con otros términos de búsqueda."
                    else:
                    ...
                    if drive_result["success"]:
                        if drive_result.get("files"):
                            ...
                            result = "No hay archivos recientes."
                    else:
                        result = f"Error: {drive_result['error']}"
...
"""
                    ...
                    if folder_result["success"]:
                        if folder_result.get("files"):
                            ...
                            result = f"La carpeta '{folder_name}' está vacía."
                    else:
            ...
            # Build messages with tool interaction
