...
def read_book_from_drive_tool(query):
    """Busca y lee libros desde Drive â€” ahora soporta PDF, EPUB y TXT."""
    files = google_drive.search_files(query)
    if not files or "No se encontraron" in str(files):
        return "No encontrÃ© ese libro en Drive."
    ...
    try:
        ...
        if lower_name.endswith('.pdf'):
            ...
            content = f"Formato '{file_name.split('.')[-1]}' no soportado. Formatos vÃ¡lidos: PDF, EPUB, TXT."
...
def generate_morning_summary(chat_id):
    """Genera resumen completo del dÃ­a: clima + agenda + tareas + titulares."""
    ...
    parts = []
    parts.append(f"â˜€ï¸ Buenos dÃ­as Pablo!")
    parts.append(f"ðŸ“… {now.strftime('%A %d de %B, %Y')} â€” {now.strftime('%H:%M')}")
    parts.append("")
    ...
    try:
        ...
        parts.append("ðŸ“‹ AGENDA DE HOY:")
        if events and "No hay eventos" not in str(events):
            parts.append(str(events))
        else:
            parts.append("Sin eventos programados. DÃ­a libre. ðŸŽ‰")
        parts.append("")
    except Exception as e:
        logger.error(f"Morning calendar error: {e}")
        parts.append("ðŸ“‹ No pude consultar el calendario.")
        parts.append("")

    # Tareas pendientes
    try:
        tasks = google_tasks.list_tasks(False)
        parts.append("âœ… TAREAS PENDIENTES:")
        if tasks and "No hay tareas" not in str(tasks):
            parts.append(str(tasks))
        else:
            parts.append("Â¡Sin tareas pendientes!")
        parts.append("")
    except Exception as e:
        logger.error(f"Morning tasks error: {e}")
        parts.append("âœ… No pude consultar las tareas.")
        parts.append("")

    # Titulares rÃ¡pidos
    if google_search_func:
        try:
            parts.append("ðŸ“° TITULARES:")
            for topic in NEWS_TOPICS[:3]:
                try:
                    for result in google_search_func(topic, num_results=1, advanced=True, lang="es"):
                        ...
                        parts.append(f"â€¢ [{clean_topic}] {result.title}")
                        break
                except:
                    pass
            parts.append("")
    ...
    parts.append("Â¿En quÃ© te puedo ayudar hoy? ðŸ’ª")
    return "\n".join(parts)
...
async def execute_tool_async(tool_name: str, tool_input: dict, chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    if tool_name == "get_current_weather":
        ...
        if not results: return f"âŒ No encontrÃ© a '{query}'."
        contact = results[0]
...
"""
       ...
       # Loop de herramientas (hasta 5 rondas)
        ...
        for round_num in range(max_tool_rounds):
            ...
            # Si agotÃ³ las 5 rondas, extraer lo que haya
            for block in current_response.content:
                if block.type == "text":
                    final_text += block.text
        ...
        if is_voice and elevenlabs_client:
            try:
                ...
                logger.info(f"ðŸ”Š Generando voz ElevenLabs...")
                ...
                logger.info("ðŸ”Š Voz enviada OK")
            except Exception as e:
...
async def check_reminders(context: ContextTypes.DEFAULT_TYPE):
    """Se ejecuta periÃ³dicamente. Revisa tareas y eventos prÃ³ximos."""
    ...
    # Eventos en las prÃ³ximas 2 horas
    try:
        ...
        if events and "No hay eventos" not in str(events):
            parts.append(f"â° PrÃ³ximos eventos (2h):\n{events}")
    except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("ðŸ‘‹ Soy Claudette V10. EPUB + Noticias + Buenos DÃ­as + Recordatorios.")
...
async def cmd_buenos_dias(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Buenos dÃ­as error: {e}")
        await update.message.reply_text(f"âš ï¸ Error generando resumen: {e}")
...
async def cmd_noticias(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if len(news) > 4000:
            news = news[:4000] + "\n\n[... Truncado. Pide un tema especÃ­fico.]"
        await update.message.reply_text(news)
    except Exception as e:
        logger.error(f"Noticias error: {e}")
        await update.message.reply_text(f"âš ï¸ Error buscando noticias: {e}")

# NUEVO: Comando /memoria para ver quÃ© recuerda
async def cmd_memoria(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    all_facts = get_all_facts() or {}
    if not all_facts:
        await update.message.reply_text("ðŸ§  Memoria vacÃ­a. Dime cosas y las recordarÃ©.")
        return
    ...
    if not lines:
        await update.message.reply_text("ðŸ§  Memoria vacÃ­a (solo tengo datos del sistema).")
        return
...
def main():
    ...
    try:
        if OWNER_CHAT_ID and app.job_queue:
            ...
            logger.info(f"ðŸ”” Recordatorios activados para chat_id: {OWNER_CHAT_ID}")
        else:
    ...
    print("âœ… Claudette Online (V10 - EPUB + Noticias + Buenos DÃ­as + Recordatorios)")
    app.run_polling()
